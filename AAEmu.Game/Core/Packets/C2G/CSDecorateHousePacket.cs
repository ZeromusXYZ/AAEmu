...
namespace AAEmu.Game.Core.Packets.C2G
{
    public class CSDecorateHousePacket : GamePacket
    {
        ...
        public override void Read(PacketStream stream)
        {
            ...
            _log.Debug("DecorateHouse, houseId: {0}, designId: {1}, x: {2}, y: {3}, z: {4}, rot {5}, objId: {6}, itemId: {7}", houseId, designId, x, y, z, quat, objId, itemId);

            if (!HousingManager.Instance.DecorateHouse(Connection.ActiveChar, houseId, designId, posVec, quat, objId, itemId))
            {
                Connection.ActiveChar.SendErrorMessage(ErrorMessageType.HouseCannotDecorate);
                _log.Warn("DecorateHouse, FAILED with houseId: {0}, designId: {1}, x: {2}, y: {3}, z: {4}, rot {5}, objId: {6}, itemId: {7}", houseId, designId, x, y, z, quat, objId, itemId);                
            }
        }
    }
}
